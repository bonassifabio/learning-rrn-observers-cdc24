...
from s5.nets import S5Model
from s5.units import S5Cell
...
class PolesZerosGainCallback(pl.Callback):
    ...
    def on_validation_end(self, trainer: pl.Trainer, pl_module: S5Model) -> None:
        if trainer.current_epoch % self.log_every_n_epochs == 0:
            poles, zeros, gains, kappa, theta = pl_module.poles_zeros_gains()
            skip_norm = pl_module.skip_norm()

            for i, (p, z, k, t, r, sn) in enumerate(zip(poles, zeros, gains, kappa, theta, skip_norm)):
                ...
                trainer.logger.experiment.add_scalar(f'SkipNorm-Mag/Layer {i}', sn, global_step=trainer.global_step)
                trainer.logger.experiment.add_scalar(f'Eig-Scaling/Layer {i}', t, global_step=trainer.global_step)
                trainer.logger.experiment.add_scalar(f'Eig-Rotation/Layer {i}', r, global_step=trainer.global_step)
