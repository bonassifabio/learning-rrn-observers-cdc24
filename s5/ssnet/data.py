...
def tbptt(training: ExperimentData, 
    ...
    Ttr = Ts_train
    Tva = min([t.shape[0] for t, _ in val_list]) - max(context_window, 0) if Ts_val < 0 else Ts_val
    ...
    for i, (exp, idx) in enumerate(zip(sampled_val_exp, sampled_val_idx, strict=True)):
        ...
        if context_window > 0:
            X0_val[i] = torch.cat([val_list[exp][0][idx-context_window:idx],
                                   val_list[exp][1][idx-context_window:idx]], dim=-1)
